...
export class PlayerComponent{
  ...
  updateTime(event): void{
    const totalWidth = document.getElementsByClassName('fullBar')[0].clientWidth;
  ...
  changeSongInfo(): void{
    ...
    const length = this.audioService.getSongTotalTime(this.audioService.index);
    this.songLength = this.convertTime(length);

    const timeLeft = length - this.audioService.getSongTimeLeft(this.audioService.index)
